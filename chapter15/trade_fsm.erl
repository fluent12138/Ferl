...
trade(OwnPid, OtherPid) ->
  io:format("tarde : ~p ~p ~n", [OwnPid, OtherPid]),
  gen_fsm:sync_send_event(OwnPid, {negotiate, OtherPid}, 30000).

%% 接受某个玩家的交易请求
accept_trade(OwnPid) ->
  io:format("accept_trade : ~p ~n", [OwnPid]),
  gen_fsm:sync_send_event(OwnPid, accept_negotiate).

%% 从物品列表中选择一个进行交易
make_offer(OwnPid, Item) ->
  io:format("~p make_offer : ~p ~n", [OwnPid, Item]),
...
ready(OwnPid) ->
  io:format("ready : ~p ~n", [OwnPid]),
  gen_fsm:sync_send_event(OwnPid, ready, infinity).

%% 取消交易
cancel(OwnPid) ->
  io:format("cancel : ~p ~n", [OwnPid]),
...
%% 当在negotiate状态时, 接收到他方的ready信号, 直接拒绝.
